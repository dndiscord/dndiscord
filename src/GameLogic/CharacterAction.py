...
class CharacterAction(GenericGameLogic):
    ...
    async def do_action(self, message):
        ...
        item_use_actions = Constants.organized_action_vocab['item_use']['character_interact']
        item_use_actions.extend(Constants.organized_action_vocab['item_use']['item_interact'])
    ...
    async def do_item_action(self, action, item_name, target_name, character, channel):
        ...
        if item is None:
            ...
            return
        await self.printMethod(channel, character.use_item(item, target, action, self.data))

    async def do_non_item_action(self, name, action, character, channel):
        target = await self.get_target(name,channel)
        if target is None:
            return
        if action in Constants.organized_action_vocab['non_item_use']['character_interact'] and not isinstance(target, Character):
            await self.printMethod(channel, "You can't {} with an inanimate object".format(action))
            return
        await self.printMethod(channel, character.use_target(target, action, self.data))
